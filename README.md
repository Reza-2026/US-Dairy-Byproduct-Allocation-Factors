# Transparent and Harmonized Allocation Factors for Crop-Processing Byproducts

[![License: CC BY 4.0](https://img.shields.io/badge/License-CC_BY_4.0-lightgrey.svg)](https://creativecommons.org/licenses/by/4.0/)
[![Status: Fact-Checked](https://img.shields.io/badge/Status-Fact--Checked-success.svg)]()

## Project Overview

This repository provides **harmonized economic, dry-matter mass, and gross energy allocation factors** for co-products derived from **22 U.S. crop-processing systems**, yielding factors for **65 dairy-relevant co-products**. These factors are rigorously documented and designed for use in dairy and livestock environmental footprinting, particularly Life Cycle Assessment (LCA) studies.
...
- **`/methodologies/`**: Contains ~22 deep-dive `.md` files—one per crop system (e.g., Corn, Soybeans, Cotton, Sugar Beets, Rice, Peanuts). Each file documents process flows, yield derivations, dry matter (DM) balances, price verifications, and allocation calculations in exhaustive detail.
- **`/data/`**: Contains master summary tables (`.csv`) with final economic, mass, and gross energy allocation factors, plus Monte Carlo uncertainty bounds.
- **`/scripts/`**: Contains Python scripts for Monte Carlo simulations (10,000 iterations per system) and cross-method divergence index calculations.

## Methodological Standards

All methodology files adhere to the following rigorous standards:

- **ISO 14044 Hierarchy**: Prioritizes physical relationships (mass/energy) and economic valuation. Multi-stage processes (e.g., Cotton ginning → crushing, Rice hulling → milling, Peanut shelling → crushing) utilize **two-stage cascade allocation** to prevent double-counting and properly handle intermediate products.
- **Strict Mass Balance Verification**: Every system includes step-by-step Dry Matter (DM) input-output reconciliation. Unallocated losses (e.g., dust, moisture evaporation, wastewater organics) are explicitly quantified.
- **Midpoint Rule**: All chosen yield and price parameters are verified to be the exact mathematical midpoints of their stated, evidence-backed ranges.
...
Every file in this repository has undergone **line-by-line forensic fact-checking** to ensure data integrity:

- **Elimination of AI Hallucinations**: All claims and figures have been manually verified against source documents.
- **No Ghost Database Misattributions**: All citations (USDA ERS, NASS, AMS, FAO, Feedipedia, industry associations) have been verified for existence, correct titles, and accurate data attribution.
- **Mass-Balance Violation Checks**: Every system's DM inputs and outputs have been reconciled to ensure physical consistency.

## 🌾 Crop Systems Covered
...
*(Navigate to the `/methodologies/` folder to view the full mass-balance and allocation math for each system.)*
...
> [Author Last Name], [First Initial]., et al. (2026). *Transparent and Harmonized Allocation Factors for Crop-Processing Byproducts* [Data set]. Zenodo. https://doi.org/10.5281/zenodo.[XXXXXXX]
